...
The list of possible candidates (candidate alternatives) is shown in a separate buffer called =*Completions*= (normally by pressing TAB key) if there are more than =completion-cycle-threshold= candidates. There are two completion styles used in the minibuffer:
...
;;;###autoload
(defun j-minibuffer-focus-minibuffer ()
...
;;;###autoload
(defun j-minibuffer-focus-minibuffer-or-completions ()
...
(use-package minibuffer
  ...
  (("s-f" . find-file)
   ...
   ("s-v" . j-minibuffer-focus-minibuffer-or-completions)))
#+end_src
