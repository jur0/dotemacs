...
The occur and replace operations are aware of active region, so the search and replace operation is executed only in the highlighted area (also possible to be done with narrowing =C-x n C-h=).
...
;;; j-embark.el --- Extensions for search, replace and grep -*- lexical-binding: t -*-
